...
export const RequestForm = ({ onSuccess }: { onSuccess?: () => void }) => {
    ...
    const [type, setType] = useState<'VACATION' | 'EXPENSE' | 'EQUIPMENT'>('VACATION');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createApprovalRequest({ title, type });
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 shadow rounded-lg">
            ...
            <div>
                ...
                <select
                    value={type}
                    onChange={(e) => setType(e.target.value as 'VACATION' | 'EXPENSE' | 'EQUIPMENT')}
